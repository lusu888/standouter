...
- (id)initWithFrame:(CGRect)frame {
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playv:) name:@"Notification_playv" object:nil];
    if ( (self = [super init]) ) {
...
-(void)playv:(NSNotification*) note{
    [self stop];
    NSLog(@"STOP");
    NSDictionary *theData = [note userInfo];
    if (theData != nil) {
       int S = [[theData objectForKey:@"row"] intValue];
        NSString *videourl = [[[_controls.resultJSON objectForKey:@"items"] objectAtIndex:S] objectForKey:@"videoUrl480"];
        NSLog(videourl);

        [self setContentURL:[NSURL URLWithString:videourl]];
...
-(void)setstandouter:(BOOL*)willshow videoid:(NSInteger*)vidno videotitle:(NSString*)vidtitle ownerid2:(NSInteger*)ownerid2 hengjson:(NSDictionary*)hengjson {
    _controls.willshow = willshow;
    _controls.videoidno=vidno;
    _controls.videotitle=vidtitle;
    _controls.ownerid=ownerid2;
    _controls.resultJSON=hengjson;
    [_controls hengtableinit];
}
